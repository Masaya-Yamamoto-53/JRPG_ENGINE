...
void EnemyAnimation::loadImages(const std::string& baseDir, const std::string& id) {
...
const CharacterImage& EnemyAnimation::getImage() const {
    return m_images[m_animIndex];
}

int EnemyAnimation::getMoveAmount() const {
    return (m_running ? RunSpeed : WalkSpeed);
}

void EnemyAnimation::updateAnimation(Direction useDir, bool isMoving) {
    if(isMoving) {
...
int EnemyAnimation::calcAnimIndex(Direction dir) const {
    int base = 0;
    switch (dir) {
    case Direction::Down:
        base = m_down;
...
int EnemyAnimation::getSpriteWidth() const { return m_spriteWidth; }
int EnemyAnimation::getSpriteHeight() const { return m_spriteHeight; }
